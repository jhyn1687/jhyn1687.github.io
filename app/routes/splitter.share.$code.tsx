...
import type { Bill, SharedBill } from "~/components/splitter/types";
...
export async function clientLoader({
  params,
}: Route.ClientLoaderArgs): Promise<{ sharedBill: SharedBill | null; error?: string }> {
  const { code } = params;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      try {
        const raw = localStorage.getItem(SHARED_KEY);
        const bills: SharedBill[] = raw ? JSON.parse(raw) : [];
        localStorage.setItem(SHARED_KEY, JSON.stringify(bills.filter((b) => b.shareCode !== code)));
      } catch { /* non-fatal */ }
      return { sharedBill: null, error: "This share link has expired or doesn't exist." };
    }
    ...
    try {
      ...
      localStorage.setItem(SHARED_KEY, JSON.stringify([...filtered, sharedBill]));
    } catch { /* non-fatal */ }
    return { sharedBill };
  } catch {
