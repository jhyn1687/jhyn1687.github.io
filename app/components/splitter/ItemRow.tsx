    ...
    <div className="overflow-hidden rounded-xl border border-ctp-surface1/50 transition-colors hover:border-ctp-surface2">
      ...
      <div className="flex items-center gap-2 px-3.5 py-2.5">
        ...
        {!readOnly && (
          ...
          >
            {confirmDelete ? <MdDeleteForever size={18} /> : <MdDelete size={18} />}
          </button>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-ctp-surface1/50 bg-ctp-mantle/50 px-3.5 py-2">
        <span className="text-[11px] font-medium text-ctp-overlay0">Split between:</span>
        {participants.length === 0 ? (
          <span className="text-[11px] italic text-ctp-overlay0">Add people first</span>
        ) : (
          <>
            {participants.map((p) => (
              <ParticipantTile
