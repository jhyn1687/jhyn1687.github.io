...
import type { Bill, LocalBill, SharedBill, SplitterSettings } from "./types";
...
function loadLocalBills(): LocalBill[] {
  try {
    ...
    if (draftRaw) {
      ...
      const migrated: LocalBill = { id: crypto.randomUUID(), bill, updatedAt: Date.now() };
      const existing = JSON.parse(localStorage.getItem(LOCAL_KEY) ?? "[]") as LocalBill[];
