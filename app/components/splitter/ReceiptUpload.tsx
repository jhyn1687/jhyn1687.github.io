...
import { parseReceiptText, type OcrItem } from "./parseReceiptText";
...
export function ReceiptUpload({ onItemsImported, hasContent }: ReceiptUploadProps) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-ctp-surface1/50 bg-ctp-surface0/40">
      ...
      >
        ...
        <span className="flex-1 text-[15px] font-bold text-ctp-text">Scan Receipt</span>
        <MdChevronRight
          size={18}
          className={[
        ...
        <div className="border-t border-ctp-surface1/50 p-4">
          {loading ? (
            ...
            <>
              ...
              {status && (
                <p className="mt-3 text-center text-xs text-ctp-subtext1">{status}</p>
              )}
            </>
          )}
          <input
